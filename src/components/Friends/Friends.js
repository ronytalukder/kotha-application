...
const Friends = () => {
  ...
  const handleActiveSingle = (item) => {
    if(item.reciverId===data.uid){
      ...
      localStorage.setItem('activeChatSingle', JSON.stringify({status:'single', id:item.senderId, name: item.senderName}))
    }
    else{
      dispatch(activeChat({ status:'single', id:item.reciverId, name: item.reciverName}));
      localStorage.setItem('activeChatSingle', JSON.stringify({ status:'single', id:item.reciverId, name: item.reciverName}))
    }
